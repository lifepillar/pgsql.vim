" Vim indent file
" Language:    PL/pgSQL
" Author:      David Fishburn <fishburn at ianywhere dot com>
...
setlocal indentexpr=GetPGSQLIndent()

" Only define the functions once.
if exists("*GetPGSQLIndent")
    finish
endif
...
let s:PGSQLBlockStart = '^\s*\%('.
            \ 'if\|else\|elseif\|elsif\|'.
                \ 'while\|loop\|do\|'.
                \ 'begin\|'.
                \ 'case\|when\|merge\|exception'.
                \ '\)\>'
let s:PGSQLBlockEnd = '^\s*\(end\)\>'
...
function GetPGSQLIndent()
    ...
    if prevline =~? s:PGSQLBlockStart
        " Move indent in
        let ind = ind + &sw
        " echom 'prevl - PGSQLBlockStart - indent ' . ind . '  line: ' . prevline
    elseif prevline =~ '[()]'
        if prevline =~ '('
